...
public class Sotilgan extends Application {
    Stage stage;
    Scene scene;
    ...
    public Sotilgan(Connection connection, User user) {
    ...
    private ObservableList<Hisob> getTableData(Connection connection) {
        ...
        select = "Select tovar, sum(dona) as dona, sum(narh*dona/kurs) as narh from hisobkitob where tovar>0 and hisob2 in(" + whereString + ") group by tovar order by tovar";
        rs = hisobKitobModels.getResultSet(connection, select);
        try {
            while (rs.next()) {
        ...
        select = "Select tovar, sum(dona) as dona, sum(narh*dona/kurs) as narh from hisobkitob where tovar>0 and hisob1 in(" + whereString + ") and manba>0 group by tovar order by tovar";
        rs = hisobKitobModels.getResultSet(connection, select);
        try {
            while (rs.next()) {
                Standart tovar = GetDbData.getTovar(rs.getInt(1));
