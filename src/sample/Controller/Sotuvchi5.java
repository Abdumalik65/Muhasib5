        ...
        narh.setOnEditCommit((TableColumn.CellEditEvent<HisobKitob, Double> event) -> {
            ...
            if (newValue != null) {
                ...
                if (narhComboBox.getValue().getId() == 1) {
                    ...
                    narhYoz(tovar.getId(), 2, yangiNarhDouble);
                }
            }
...
//        stage.initStyle(StageStyle.UNDECORATED);
    ...
    private void tolovChiptasiniBerXP80() {
        ...
        printerService.printString("XP-80", chipta);
    ...
    private Kurs getKurs(int valutaId, Date sana) {
        ...
        Kurs kurs = null;
        if (v.getStatus() == 1) {
            kurs = new Kurs(1, new Date(), valutaId, 1.0, user.getId(), new Date());
        } else {
    ...
    private Double getKurs(Valuta valuta) {
        Double kurs = 1d;
        if (valuta.getStatus().equals(1)) {
            kurs = 1d;
        } else {
            Kurs kurs1 = getKurs(valuta.getId(), new Date());
            kurs = kurs1.getKurs();
        }
        return kurs;
    }
    ...
    private void addTovar(BarCode barCode) {
        ...
        double kursDouble = getKurs(valuta.getId(), hisobKitob.getDateTime()).getKurs();
        hisobKitob.setKurs(kursDouble);

        double pochkadaDona = tovarDonasi(barCode);
        Double narhDouble = narhOlDouble(tovar.getId(), narhTuri+1);
        if (narhDouble == null) {
            Alerts.kamomat(tovar);
            return;
        }

        double narhDoubleFinal = 0d;
        double nds = getNds(tovar.getId());
    ...
    private Double getNds(int tovarId) {
        Double ndsDouble = 0d;
        standart4Models.setTABLENAME("Nds");
        Standart4 ndsStandart4 = standart4Models.getTartibForDate(connection, tovar.getId(), new Date(), "dateTime desc");
        if (ndsStandart4 != null) {
            ndsDouble = ndsStandart4.getMiqdor() * 0.01;
        }
        return ndsDouble;
    }
    ...
    private void addTovarToTable(BarCode barCode, HisobKitob hisobKitob) {
        double tovarDonasi = tovarDonasi(barCode);
        double adad = 1d;
        Standart tovar = GetDbData.getTovar(hisobKitob.getTovar());
        Double zaxiradagiAdad = hisobKitobModels.getTovarBalansDouble(connection, hisobKitob.getHisob2(), tovar, new Date())/tovarDonasi;
        HisobKitob hk1 = null;
        for (HisobKitob hk : tableViewObservableList) {
            if (hk.getBarCode().equals(barCode.getBarCode())) {
                hk1 = hk;
                if (zaxiradagiAdad >= hk.getDona() + adad) {
                    adad = adad + hk.getDona();
                    hk1.setDona(adad);
                    break;
                } else {
    ...
    public Double narhOlDouble(int tovarId, int narhTuri) {
        Double narhDouble = null;
        Standart6 s6 = null;
        Standart6Models standart6Models = new Standart6Models("TGuruh1");
        Standart3Models standart3Models = new Standart3Models();
        standart3Models.setTABLENAME("TGuruh2");
        ...
        if (s3List.size()>0) {
            ...
            switch (narhTuri) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    narhDouble = s6.getBoj();
                    break;
                case 5:
                    narhDouble = s6.getNds();
                    break;
            }
        }
        ...
        if (narhDouble == null) {
            ...
            if (observableList.size()>0) {
                TovarNarhi tovarNarhi = observableList.get(0);
                narhDouble = tovarNarhi.getNarh();
            }
        }
        return narhDouble;
    }
    ...
    private BarCode barCodeDonabay(BarCode barCode) {
        ...
        if (tarkibInt>0) {
            while (true) {
                ...
                dona *= adadBarCode2;
                tarkibInt = barCode2.getTarkib();
                if (adadBarCode2 == 1.0 && tarkibInt > 0) {
                    barCode1 = barCode2;
                    break;
                }
            }
