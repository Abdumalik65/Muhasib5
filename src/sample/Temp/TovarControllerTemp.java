...
import sample.Model.TovarNarhiModels;
import sample.Tools.*;
...
public class TovarControllerTemp extends Application {
    Stage stage;
    Scene scene;
    ...
    public TovarControllerTemp() {
    }

    public TovarControllerTemp(Connection connection, User user) {
    ...
    private void initData() {
        ...
        tovarObservableList = FXCollections.observableArrayList();
        if (barCodesResult.size()>0) {
            for (BarCode bc: barCodesResult) {
                if (getTovar(bc.getTovar(), tovarObservableList) == null) {
...
;                    }
    ...
    private Standart getTovar(Integer tovarId, ObservableList<Standart> tovarObservableList) {
        Standart tovar = null;
    ...
    private void initBarCodeImageViewButton() {
        barCodeImageViewButton.setOnAction(event -> {
            ...
            if (bc != null) {
                ...
                BufferedImage bufferedImage = toBufferedImage(code128Image);
...
/*        textColumn.setCellFactory(TextFieldButtonTableCell.<BarCode> forTableColumn());
        ...
        textColumn.setOnEditCommit((TableColumn.CellEditEvent<BarCode, String> event) -> {
            ...
            BarCode barCode = event.getRowValue();
            BarCode newBarCode = barCodeModels.getBarCode(connection, newString);
